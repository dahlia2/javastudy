...
public class User {
	...
	*/
	...
	/*
		정보 은닉(information Hiding)  - > private
		 1. 클래스의 내부 정보를 외부에 노출하지 않는 것을 말한다.
		 2. 클래스의 내부 정보는 "필드"를 의미하므로 "필드"값을 외부에 공개하지 않는 것을 말한다.
		 3. "필드"는 private 하게 처리한다.  -> MainClass에서 노출시키면 안 됨. 그렇다면?  ↓
	...
	*/
	...
	/*
	 	...
	 	1. Getter
	 		1) 필드값을 외부로 반출하는 메소드
	 		2) get + 필드이름 을 가지는데 이를 수정하면 안 된다.
	...
	*/
	...
	/*
		boolean 타입은 get 대신 is[필드명]으로 나옴.
	    ㄴ필드명 앞에 is[필드명]으로 지어도 isis[필드명]으로 생성 X
	*/
