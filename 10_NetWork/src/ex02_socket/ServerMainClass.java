...
public class ServerMainClass {
		...
		try {
			...
			// 쉽게 말하면 주소를 알아야 통신할 수 있기 때문에 사용
			InetSocketAddress address = new InetSocketAddress("localhost", 9090); // 완성되는 주소 - local host:9090
			...
			while(true) {
				...
				System.out.println("[서버 /개발자/화면] 클라이언트 접속을 기다리고 있습니다.");
				
				// 클라이언트의 접속 요청을 허용
				Socket clientSocket = serverSocket.accept();
				// 클라이언트의 접속 확인을 위해서 InetSocketAddress를 활용
				InetSocketAddress clientAddress = (InetSocketAddress)clientSocket.getRemoteSocketAddress();
				System.out.println("[서버 /개발자/화면] 접속된 클라이언트 : " + clientAddress.getHostName());
				
				// 클라이언트에게 welcome 메세지 보내기
				...
				BufferedReader in = new BufferedReader(new InputStreamReader(clientSocket.getInputStream()));
				            // 3. 속도 향상을 위해 버퍼착용   <-  2. 문자로 읽을 수 있게 변환하고   <-   1. 바이트 스트림을 받고
				String line = null;
				StringBuilder sb = new StringBuilder();
				while((line = in.readLine()) != null) {
					sb.append(line + "\n");
				}
				System.out.println(("[서버 /개발자/화면] " + sb.toString()));
