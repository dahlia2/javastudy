...
public class MainClass {
	...
	*/
	...
	/* 
		...
		3. 문자를(데이터를) 내보내는 스트림이다. (파일로 문자를 보낸다. 서버로 문자를 보낸다. 등등)
	*/
	...
	public static void ex01() {
		...
		try {
			...
			fw.write(cbuf);
			fw.write(str);
			// write.(int 타입에 char 가능) => int는 4바이트 char는 2바이트이므로 자동변환
			...
			try {
				if(fw != null) {
					fw.close();  // 반드시 예외처리가 필요한 코드. (try문에 있어야 함)
				}
			} catch(IOException e) {
				e.printStackTrace();
			}
	...
	*/
		...
		try {
			...
			try {
				// 보조 스트림을 사용한 경우네는 보조 스트림만 닫으면 메인 스트림도 함께 닫힌다.
				if(bw != null) {
					bw.close();
				}
			} catch(IOException e) {
	...
	public static void ex05() {
		...
		*/
		...
		try {
			...
			out.close(); // close 메소드 내부에서 이미 IOExeption을 처리하고 있음.
		}
	}
